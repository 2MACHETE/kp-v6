...
/*
 * Структура для хранения данных о котле:
 * K - номер котла
 * indicator - разница в процентах между введенным давлением и рассчитанным
 */
typedef struct
{
    int K;
...
data data_array[MAX_DATA_SIZE]; // Массив для хранения данных
int data_count = 0;             // Счетчик данных в массиве
...
void main()
{
    ...
    while (1)
    {
        ...
        printf("8) Очистить массив данных\n");
        printf("9) Отсортировать данные\n");
        printf("10) Выход\n");
        scanf("%d", &int_menu);
        switch (int_menu)
        {
        ...
        case 7:
            ...
            break;
        case 8:
            clear_data_array();       // Очистка массива данных
            break;
        case 9:
...
void write_array(int K, params params1)
{
    ...
    {
        data_array[data_count].K = K;
        data_array[data_count].indicator = indicator(params1);
        data_count++;
    }
    else {
...
/*
 * Функция для очистки массива данных
 * Сбрасывает счетчик данных в массиве
 */
void clear_data_array()
{
    data_count = 0;
    printf("Массив данных очищен\n");
    return;
}
